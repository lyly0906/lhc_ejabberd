...
var CompanyManager = {
		...
		createCompanyItem :function(obj){   
			...
			var itemHtml = "<div id='company_"+obj.id+"'>"
							   ...
							   + "<tbody>"
	                            ...
	                            +     "</td>"
	                            +     "<td width='80%'>"
	                            +		  "<h6 id='companyName' class='media-heading' style='font-weight:bold; font-size:13px;'>" +obj.companyName+ "</h6> "
								...
								+  		 	    (myFn.isNil(obj.noticeContent) ? "暂无公告" : obj.noticeContent)   
								+  	 	     "</a>"
								+    	   "</div>"
	                            +     "</td>"
	                            +  	  "<td width='10%'>"
	                            +          "<img  src='img/menuBar.png' class='companyMenuBar' onclick='CompanyManager.showCompanyMenu(\"" + obj.id + "\");'>" 
	                            +     "</td>"
	                            +  "</tr>"
	                            +"</tbody>"
	                           +"</table>"
	                           +"<div id='companyMenu_"+obj.id+"' class='companyMenu' style='display:none;  position:absolute; right:1%;' onmouseout='CompanyManager.hideCompanyMenu(\"" + obj.id + "\",event);'>";
	                           
	                           if (employeeRole[obj.id]>=2)  //权限判断
							   		itemHtml += "<a href='#' onclick='CompanyManager.showCreateDepartment(\"" + obj.id + "\")'>创建部门</a>"
							   				+"<a href='#' onclick='CompanyManager.modifyCompanyInfo(\"" + obj.id + "\")'>修改信息</a>"
							   				+"<a href='#' onclick='CompanyManager.setAdministrator(\"" + obj.id + "\")'>设置管理员</a>";


							   itemHtml += "<a href='#' onclick='CompanyManager.modifyMyPosition(\"" + obj.id + "\")'>我的职位</a>"
							   +"<a href='#' onclick='CompanyManager.quitCompany(\"" + obj.id + "\")'>退出公司</a>"  
							   +"</div>"
			...
			for (var i = 0; i < length; i++) {
				...
				if (department.parentId == rootDepartId) {
					...
					var departHtml = "<div id='department_"+department.id+"'> "
							        + 	"<div class='' style='padding: 8px; border-bottom: 1px solid #ddd; background: #fff'>"                                 
							        ...
							        +   "</div>"  
							        +   "<div id='departMenu_"+department.id+"' class='departMenu' style='position: absolute; right: 1%; display: none;' onmouseout='CompanyManager.hideDepartmentMenu(\"" + department.id + "\",event);'>" 

							        if (employeeRole[department.companyId]>=1) //权限判断
							        	departHtml += "<a href='#' onclick='CompanyManager.showCreateChildDepartment(\"" +department.companyId+ "\",\"" + department.id + "\")'>创建子部门</a>"
												   + "<a href='#' onclick='CompanyManager.delEmployee(\"" +department.companyId+ "\",\"" + department.id + "\")'>删除员工</a>"
							        			   + "<a href='#' onclick='CompanyManager.quitDepartment(\""+department.id+"\",\""+department.companyId+"\")'>删除部门</a>";

							        departHtml += "<a href='#' onclick='CompanyManager.addEmployee(\"" + department.id + "\",\"" + department.companyId + "\")'>添加员工</a>"
							        +   "</div>"
		...
		createEmployeeItem : function(employees){ 
			...
			for (var i = 0; i < employees.length; i++) {
			  	...
			  	var empHtml = "<div id='emp_"+employee.userId+"' class='media-main'>"
			  	 				...
			  	 				+	"</a>" 
			  	 				+   "<div onclick='ConversationManager.open(\"" + (employee.userId + "@" + AppConfig.boshDomain) + "\",\"" + employee.nickname+ "\");' style='cursor: pointer;' class='media-body'>"
			  	 				+   	"<h5 class='media-heading'>" + employee.nickname + "</h5>"
		...
		quitCompany : function(companyId){  //退出公司
			...
			if (role == 3) {
				...
				window.wxc.xcConfirm("您是该公司的创建者，此操作会将公司删除,是否继续？", window.wxc.xcConfirm.typeEnum.confirm,{
					onOk:function(){
						myFn.invoke({
			...
			myFn.invoke({
				...
				success : function(result) {
					if (1 == result.resultCode) {
						...
						var departHtml = "<div id='department_"+department.id+"'> "
							        + 	"<div class='' style='padding: 8px; border-bottom: 1px solid #ddd; background: #fff'>"   // class='media-heading'                                
							        +   	"<img id='departImg_"+department.id+"' class='departmentImg'  src='img/unfold.png' onclick='CompanyManager.createChildDepartItem(\"" + department.companyId+ "\",\"" + department.id + "\");'> " 
							        +          department.departName 
							        + 	    "<img class='departMenuBar' src='img/menuBar.png' onclick='CompanyManager.showDepartMenu(\"" + department.id + "\");'>" 
							        +   "</div>"  
				...
				success : function(result) {
					if (1 == result.resultCode) {
						...
						var childDepartHtml = "<div id='department_"+department.id+"'> "
							        ...
							        if (employeeRole[department.companyId]>=1) //权限判断
							        	childDepartHtml += "<a href='#' onclick='CompanyManager.showCreateChildDepartment(\"" +department.companyId+ "\",\"" + department.id + "\")'>创建子部门</a>"
							        			   +"<a href='#' onclick='CompanyManager.delEmployee(\"" +department.companyId+ "\",\"" + department.id + "\")'>删除员工</a>"
							        			   +"<a href='#' onclick='CompanyManager.quitDepartment(\""+department.id+"\",\""+department.companyId+"\")'>删除部门</a>";

							        childDepartHtml += "<a href='#' onclick='CompanyManager.addEmployee(\"" + department.id + "\",\"" + department.companyId + "\")'>添加员工</a>"; 
							        +   "</div>"
							        + 	"<div id='childDepartList_"+department.id+"' class='childDepartList'> </div>"
							        + 	"<div id='employeeList_"+department.id+"' style='display: none;'></div>"
						        	+"</div>";				
		...
		modifyMyPosition : function(companyId){ //修改我的职位
			...
			$("#positionModify").click(function() {  //点击修改
				var newPositionName = $("#modify_myPosition  #positionName").val(); //获取职位
				//首先判断职位是否为空
					...
					myFn.invoke({
						...
						success : function(result) {
							if (1 == result.resultCode) {
								...
								$("#modify_companyInfo").modal('hide');
								//在界面上修改公司列表中的信息
								$("#company_"+companyId+" #companyName").text(company.companyName);
								$("#company_"+companyId+" #companyNotice").text(company.noticeContent);
							} else {
								ownAlert(2,result.resultMsg);
							}
						},
					...
					if (1 == result.resultCode) {
						for (var i = 0; i < result.data.length; i++) {
							...
							departEmpHtml += "<tr><td>"
							              ...
							              +  "</td><td width=100%>&nbsp;&nbsp;&nbsp;&nbsp;" + employee.nickname
							              +  "</td><td>"
							              +	 "<input id='areadyChooseFriends' name='userId' type='checkbox'  value='" + employee.userId + "' onclick='Checkbox.checkedAndCancel(this)'/></td></tr>";
						}
		...
		joinEmploy : function(departmentId,companyId){
			if (CompanyManager.getEmployeeList().length <=0 ) {
		...
		showFriendList : function(pageIndex,companyId){
			mySdk.getFriendsList(myData.userId, null, 2, pageIndex, function(result) {
				...
				for (var i = 0; i < result.pageData.length; i++) {  //循环好友列表
					...
					if (isExist) {  
						cpFriendsListHtml += "<tr><td>"+"<img onerror='this.src=\"img/ic_avatar.png\"' src='" + imgUrl + "' width=30 height=30  class='roundAvatar'/></td><td width=100%>&nbsp;&nbsp;&nbsp;&nbsp;" + obj.toNickname
				...
				var pageHtml = CompanyManager.createPager(pageIndex, result.pageData.length, companyId, 'CompanyManager.showFriendList');
				$("#cpFriendsList").empty();
				$("#employees_page").empty();
		...
		createPager : function(pageIndex, length, parameter, fnName) {   //用于翻页
			...
			if (length < 10) {
				...
				pagerHtml += "<a href='javascript:" + fnName + "(" + (pageIndex + 1) + ",\"" + parameter + "\")" + "'><img width='15' height='15' alt='' src='img/next.png'></a>";
			}
			return pagerHtml;
		...
		quitDepartment : function(departmentId,companyId) {
			if (CompanyManager.roleDiscern(companyId,0)) {
				...
				window.wxc.xcConfirm('是否确认删除该部门?', window.wxc.xcConfirm.typeEnum.confirm,{
					onOk:function(){
