...
start(Host, Opts) ->
    ...
    supervisor:start_child(ejabberd_backend_sup, PingSpec),
    ?INFO_MSG("lhcping From intercepted ~p", [Opts]).
stop(Host) ->
    Proc = gen_mod:get_module_proc(Host, ?MODULE),
...
mod_opt_type(ahenviroment) ->
    fun iolist_to_binary/1;			
mod_opt_type(_) ->
    [iqdisc, ping_interval, send_pings, timeout_action,basedomain, ahprotocol, ahenviroment, ping_address].