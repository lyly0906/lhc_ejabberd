...
class erLhcoreClassExtensionXmppserviceHandler
{
        ...
        if ($params['handler'] == 'rpc') {
            try {
                ...
                foreach ($data as $groupData) {
                    ...
                    file_put_contents("log.txt", "-----addUser-first-------------".var_export($infoUser,true), FILE_APPEND);
		  $rpc->createUser($groupData['user'],$groupData['password']);
                    file_put_contents("log.txt", "-----addUser-second-------------".var_export($groupData,true), FILE_APPEND);
                    ...
                    file_put_contents("log.txt", "-----addUser-------------".var_export($contents,true), FILE_APPEND);
   ...
   public static function setUserCard($params){
        ...
        if ($params['handler'] == 'rpc') {
            try {
                ...
                if($params['dealtype'] == 'add'){
                    ...
                    $nowRoom = $params['name'].'@conference.'.$params['xmpp_host'];  // 查找该房间是否存在
    ...
    public function iqSendMessage($jsonContent){
        ...
        $conference     = 'conference.'.$hostname;
        $port           = 5222;
